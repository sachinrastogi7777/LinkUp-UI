...
const ReceivedRequests = (props) => {
    ...
    useEffect(() => {
        if (onLengthChange) {
            onLengthChange(requestReceivedListData?.length);
        }
    ...
    const handleAcceptRequest = (status, requestId) => {
        setLoadingRequestId(requestId);
        setProcessingType('accept');
        reviewedRequests(status, requestId).finally(() => {
    ...
    const handleDeclineRequest = (status, requestId) => {
        setLoadingRequestId(requestId);
    ...
    return (
        <div>
            {props.isLoadingReceived && !requestReceivedListData?.length ? (
                <LoadingRequests />
            ) : (
                <div className="space-y-6">
                    <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2">
                                        ...
                                        <div className="flex flex-col md:flex-row gap-4">
                                            ...
                                            <div className="flex flex-col gap-2 md:w-auto w-full items-stretch">
                                                <div className="flex flex-row gap-3 w-full">
                                                    {/* Accept Button: Hide Decline if Accept is processing */}
                                                    {!(loadingRequestId === request._id && processingType === 'decline') && (
                                                        <button
                                                            onClick={() => handleAcceptRequest("accepted", request._id)}
                                                            className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl font-medium hover:from-green-600 hover:to-emerald-600 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
                                                    ...
                                                    {/* Decline Button: Hide Accept if Decline is processing */}
                                                    {!(loadingRequestId === request._id && processingType === 'accept') && (
                                                        <button
                                                            onClick={() => handleDeclineRequest("rejected", request._id)}
                                                            className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-white border-2 border-gray-200 text-gray-700 rounded-xl font-medium hover:bg-gray-50 hover:border-gray-300 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
                                                            disabled={loadingRequestId === request._id}
                                                        >
                                                            {loadingRequestId === request._id && processingType === 'decline' ? (
                                                                <Loader className="w-4 h-4 animate-spin" />
